...
function imprimirDespesas(despesas){
    ...
    despesas.forEach((item, index, array) =>{
...
function imprimirExtrato(){
    ...
    // AQUI VEM A IMPLEMENTAÇÃO
    arrDespesas.map((item,index,array) =>{
...
function filtrarDespesas(){
    ...
    let despesasFiltradas = arrDespesas.filter((item, index, array) =>{
        if(item.tipo == tipoFiltro && item.valor >= valorMin && item.valor <= valorMax){
            return true
        }else if(tipoFiltro == "" && valorMax <= 0 && valorMin <=0){
            alert("Os campos de filtragem estão inválidos.")
        }
    })
